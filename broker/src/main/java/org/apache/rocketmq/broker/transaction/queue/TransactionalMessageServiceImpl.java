...
public class TransactionalMessageServiceImpl implements TransactionalMessageService {
    ...
    public void check(long transactionTimeout, int transactionCheckMax,
        ...
        try {
            ...
            for (MessageQueue messageQueue : msgQueues) {
                ...
                // 事务已经提交或回滚的opOffset(事务已提交或回滚，且store中halfOffset已经更新后，用于更新store中opOffset)
                List<Long> doneOpOffset = new ArrayList<>();
                // 事务已经提交或回滚的,但halfQueue中还未删除的消息的offset  结构: halfOffset-->opOffset
                ...
                while (true) {
                    ...
                    // 包含i,删除指定offset消息,通过++newOffset（最后一行）实现
                    if (removeMap.containsKey(i)) {
                        log.info("Half offset {} has been committed/rolled back", i);
                        removeMap.remove(i);
                    }

                    // removeMap不包含i, 检查是否需要发送check status 请求
                    else {
                        ...
                        // 判断事务消息的等待完成的时间是否满足check status条件
                        long valueOfCurrentMinusBorn = System.currentTimeMillis() - msgExt.getBornTimestamp();
                        long checkImmunityTime = transactionTimeout;
                        String checkImmunityTimeStr = msgExt.getUserProperty(MessageConst.PROPERTY_CHECK_IMMUNITY_TIME_IN_SECONDS);
                        if (null != checkImmunityTimeStr) {
                            checkImmunityTime = getImmunityTime(checkImmunityTimeStr, transactionTimeout);
                            // 不满足检查条件，再次确认该消息是否已经成功 ？？ TODO 此处不明白
    ...
    private PullResult fillOpRemoveMap(HashMap<Long, Long> removeMap,
        ...
        // 没有匹配的结果，则直接更改real topic的offset，说明该offset的相关事务已处理完成
        ...
        // 无新的op message，则无需要删除halfOffset，直接返回
        ...
        for (MessageExt opMessageExt : opMsg) {
            ...
            if (TransactionalMessageUtil.REMOVETAG.equals(opMessageExt.getTags())) {
                // queueOffset(opOffset) 小于最小的 halfOffset，说明该事务已完成
                if (queueOffset < miniOffset) {
                    doneOpOffset.add(opMessageExt.getQueueOffset());
                }
                // 大于最小的halfOffset, 说明halfOffset需要删除已经处理的prepare msg, 添加到待删除Map中
                else {
    ...
    /**
     * 重新构建ImmunityHalfMessage消息，放入队列中 TODO 作用不清楚
     * @param messageExt
     * @return
     */
